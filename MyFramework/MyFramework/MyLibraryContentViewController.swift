...
class MyLibraryContentViewController: UIViewController {
    ...
    init(completion: @escaping (String) -> Void) {
        ...
        guard let resourceBundleURL = frameworkBundle.url(forResource: "MyFramework", withExtension: "bundle"),
              let resourceBundle = Bundle(url: resourceBundleURL),
              let animation = LottieAnimation.named("mexico_animation", bundle: resourceBundle) else {
            fatalError("Failed to locate or load MyFramework.bundle or mexico_animation.json")
        }
